...
public class Сontainer implements Serializable  {
	...
	private   int index=-1;
	private   PoliceFile[] m_data=new PoliceFile[255];
	...
	Сontainer(Сontainer rhs)/// Конструктор
	{
	  ...
	  void add(PoliceFile File) //додає вказаний елемент до кінця контейнеру;
	 {
			if (index + 1 >= 255) return;
			m_data[++index] = (PoliceFile) File;
	    		 
	  }
	  void add_from_keyboard(PoliceFile  add_from_ )
		{
	 ...
	 void clear()/// видаляє всі елементи з контейнеру;
	 {
 	 ...
 	 void Print_run(Сontainer A, PoliceFile pr) /// спадок вивести  
 	{
	 ...
	 void dop_INFO(Сontainer A,PoliceFile h) /// Спосб додавання елементів
		{
			
			int  a;
			System.out.print("\n 1: З клавiатири\n 2: 3 Файлу(недоступно)\n Ваш вiбiр:   ");
			a=in.nextInt();
			switch (a)
			{
			case 1:
	 ...
	 void UpdateData(Сontainer A,PoliceFile h){
	 ...
	 void posuk(Сontainer container)//// помилку пошуку
	 {
	 ...
	 void Sort(Сontainer container)//// помилку пошуку
	 {
		 if (container.index==-1) {
			 System.out.print("Масив пустий");
			 return ;	
